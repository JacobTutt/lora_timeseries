...
def hyperparam_wandb(config=None):
    ...
    with wandb.init(config=config):
        ...
        save_path = 'LR_{lora_rank}_BS_{batch_size}_TS_{token_length}_LR_{learning_rate}_steps_{max_training_steps}.json'

        # ------------------------ Call Your Training Loop ------------------------ #

        _, _, _, _, _, _, _, _ = train(model=model, lora_rank=lora_rank, max_training_steps=max_training_steps, batch_size=batch_size, no_tokens = token_length ,learning_rate=learning_rate, train_dataset=train_dataset, val_dataset=val_dataset, early_stopping_patience = None,
